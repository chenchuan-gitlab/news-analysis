function createRecord() {
    ...
    if(warehouse_number==""){
        ...
        $.ajax({
            url: "warehousing/createWarehousingRecord.action",
            type: "POST",
...
function getRecordList(pageNum) {
    var warehousing_batch = $("#warehousing_batch").val();
    ...
    var operator = $("#operator").val();
    $.ajax({
        url:"warehousing/getRecordList.action",
        type:"GET",
        dataType:"json",
        contentType:"application/json;charset=UTF-8",
        data:{"warehousing_batch":warehousing_batch,"start_time":start_time,"end_time":end_time,"warehouse_number":warehouse_number,
...
function showData(object) {
    ...
    for(var i = 0;i<object.list.length;i++){
        $("tbody").append("<tr><td>" + object.list[i].warehousing_batch + "</td><td>" + object.list[i].warehousing_date + "</td><td>" +
            object.list[i].warehouse_number + "</td><td>" + object.list[i].production_batch + "</td><td>" + object.list[i].product_type + "</td><td>" +
            object.list[i].product_name + "</td><td>"+object.list[i].quantity+"</td><td>"+object.list[i].operator+"</td><td>" +
