        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-body ">
                    ...
                    <div class="layui-inline layui-show-xs-block">
                        <input type="text" name="website" placeholder="请输入网站名" autocomplete="off"
                ...
                <div class="layui-card-header">
                    <button class="layui-btn layui-btn-danger" onclick="delData(null)"><i class="layui-icon"></i>批量删除
                    </button>
                    <button class="layui-btn" onclick="xadmin.open('添加网站','./websiteConfig-add.jsp',600,400)"><i
...
<script>
    ...
    function updateStatus(id, status) {
        layer.confirm('确认要更新状态吗？', function (index) {
            $.ajax({
                url: "../website/updateStatus.action?id=" + id + "&status=" + status,
    ...
    function delData(obj) {
        ...
        if (checkedData.length == 0) {
            ...
            layer.confirm('确认要删除吗？', function (index) {
                // 发异步删除数据
                $.ajax({
                    url: "../website/delete.action?id=" + checkedData,
                    success: function (data) {
                        var code = data.code;
                        if (code === 0) {
    ...
    function search(pageNum) {
        ...
        $.ajax({
            url: "../website/list.action",
            type: "GET",
            dataType: "json",
            contentType: "application/json;charset=UTF-8",
            data: {"website": website, "startTime": startTime, "endTime": endTime, "pageNum": pageNum},
            success: function (data) {
                var code = data.code;
                if (code === 0) {
    ...
    function showData(object) {
        ...
        for (var i = 0; i < object.rows.length; i++) {
            ...
            html += "<tr><td><input type='checkbox' name='' lay-skin='primary' value='" + object.rows[i].id + "' lay-filter='itemChoose'></td><td>" +
                object.rows[i].website + "</td><td>" +
                object.rows[i].newsType + "</td><td>" +
                object.rows[i].url + "</td><td>" +
            ...
            html += "<td class='td-manage'>" +
                ...
                "<a title='编辑' onclick=xadmin.open('编辑','./websiteConfig-edit.jsp?id=" + object.rows[i].id + "',600,400) href='javascript:;'><i class='layui-icon' style='font-size: 23px'>&#xe642;</i></a>" +
